...
//			System.out.println("Touched game coordinates on x=" + touchedPoint.x + " y=" + touchedPoint.y);
			...
			canvas.drawCircle(touchedPoint.x%mTileSize, touchedPoint.y%mTileSize, 10, paint);
			mTileGrid[touchedTilePoint.x][touchedTilePoint.y][1] = copy;
			// Set these points as the previousPoints
			previousTileX = touchedTilePoint.x;
			previousTileY = touchedTilePoint.y;
			// Update the view
			invalidate();
	...
	public Bitmap[][][] getGridCopy(){
		...
		for (int x = 0; x < tileCountX; x++) {
			for (int y = 0; y < tileCountY; y++) {
				Bitmap original = mTileGrid[x][y][1];
				copy[x][y][1] = original.copy(original.getConfig(), true);
			}
		}
		return copy;
	}
